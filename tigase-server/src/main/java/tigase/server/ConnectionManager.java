		...
		if (ios != null) {
			...
			if (ios.writeInProgress.tryLock()) {
				try {
					...
					System.err.println("Exception during writing packets: "+e.getMessage());
					try {
						ios.stop();
					} catch (Exception e1) {
						log.log(Level.WARNING, ios + "Exception stopping XMPPIOService: ", e1);
						System.err.println("Exception stopping XMPPIOService: "+e.getMessage());
									...
									switch ( watchdogPingType ) {
										case XMPP:
											...
											if ( !writePacketToSocket( (IO) service, pingPacket ) ){
												...
												System.err.println("ping time out: "+service.getUserJid());
												++watchdogStopped;
												service.forceStop();
											}
											break;

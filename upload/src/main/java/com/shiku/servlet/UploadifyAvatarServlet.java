...
public class UploadifyAvatarServlet extends BaseServlet {
	...
	protected JMessage hander(HttpServletRequest request, HttpServletResponse response) {
		DiskFileItemFactory factory = new DiskFileItemFactory(1024 * 1024,
				new File(getSystemConfig().getuTemp()));
		...
		FileItem item = null;
			...
			for (int i = 0; i < multipart.size(); i++) {
				FileItem t = multipart.get(i);
				if (t.isFormField()) {
					if ("userId".equals(t.getFieldName())) {
						userId = Long.parseLong(t.getString());
					}
				} else {
					item = t.getSize() > 0 ? t : null;
				}
			}
			...
			try {
				...
				jMessage = new JMessage(1, null, new UploadItem(null,
						oUrl, tUrl));
				return jMessage;
			} catch (Exception e) {
				e.printStackTrace();
